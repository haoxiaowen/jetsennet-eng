/* 
 * Copyright (c) 北京捷成世纪科技股份有限公司. All Rights Reserved.
 * 文件：jetsennet.jdvn.service.MessageServices.java
 * 日 期：2014-2-14 下午3:33:50
 * 作 者：蒋勇
 */
package jetsennet.jdlm.services.in;

import javax.jws.WebService;
import javax.ws.rs.FormParam;
import javax.ws.rs.POST;
import javax.ws.rs.Path;

import jetsennet.frame.service.BaseService;
import jetsennet.jdlm.business.PpnSourceBusiness;
import jetsennet.net.WSResult;
import jetsennet.util.StringUtil;

@Path(value = "/PpnSourceService")
@WebService(name = "/PpnSourceService")
public class PpnSourceService extends BaseService {
	
	private PpnSourceBusiness ppnSourceBusiness;
	
	@javax.jws.WebMethod(exclude = true)
	public PpnSourceBusiness getPpnSourceBusiness() {
		return ppnSourceBusiness;
	}
	@javax.jws.WebMethod(exclude = true)
	public void setPpnSourceBusiness(PpnSourceBusiness ppnSourceBusiness) {
		this.ppnSourceBusiness = ppnSourceBusiness;
	}
	/**
     * 添加人员
     */
	@POST
	@Path(value = "/selectUsers")
	public WSResult selectUsers(@FormParam("USER_IDS") String userIds)
	{
		WSResult ret = new WSResult();
		try {
			ret = ppnSourceBusiness.selectUsers(userIds, ret);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
	}
	
	/**
	 * 保存资源出库单
	 * */
	@POST
	@Path(value = "/saveSource")
	public  WSResult saveSource(@FormParam("saveXml") String xml) throws Exception
		{
		WSResult ret = new WSResult();
		try{
			ret = ppnSourceBusiness.saveSource(xml,ret);
		}catch(Exception e){
			e.printStackTrace();
		}
			return ret;
	}
	
	 //删除资源出库单
    @POST
	@Path(value = "/removeSource")
	public WSResult removeSource(String ids) {
		WSResult retObj = new WSResult();

		try {
			ppnSourceBusiness.removeSource(ids);
			retObj.setErrorCode(0);
		} catch (Exception e) {
			retObj.setErrorCode(1);
		}
		return retObj;

	}
    
    
    /**
     * 审核
     */
	@POST
	@Path(value = "/submitSource")
	public WSResult submitSource(@FormParam("out_id") String outId)
	{
		WSResult ret = new WSResult();
		try {
		String itStr = ppnSourceBusiness.submitSource(outId, ret);
		if("OK".equals(itStr)){
			ret = getResult(itStr);
		}else{
			ret.setErrorCode(-1);
			ret.setErrorString("提交审核失败");
		}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
	}
	@POST
	@Path(value = "/exeSubmit")
	public WSResult exeSubmit(@FormParam("OUT_CODE") String outCode,@FormParam("OUT_STATUS") String outStatus)
	{
		WSResult ret = new WSResult();
		try {
		String itStr = ppnSourceBusiness.exeSubmit(outCode,outStatus, ret);
		if("OK".equals(itStr)){
			ret = getResult(itStr);
		}else{
			ret.setErrorCode(-1);
			ret.setErrorString("提交审核失败");
		}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
	}
	
	@POST
	@Path(value = "/submitSour")
	public WSResult submitSour(@FormParam("OUT_CODE") String outCode,@FormParam("OUT_STATUS") String outStatus)
	{
		WSResult ret = new WSResult();
		try {
		String itStr = ppnSourceBusiness.submitSour(outCode,outStatus, ret);
		if("OK".equals(itStr)){
			ret = getResult(itStr);
		}else{
			ret.setErrorCode(-1);
			ret.setErrorString("提交审核失败");
		}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
	}
}
