...
jetsennet.ui.GridList = function (instanceId) {
    ...
    this.enableMultiSelect = false;                                      // 是否可以多选
    ...
    for(var i=1; i<rows.length; i++) {
        ...
        for(var j=0; j<cels.length; j++) {
            ...
            if(col.isCheck) {
                ...
                datasource.push(jetsennet.xml.htmlEscape(cels[j].innerHTML));
            }
            datasource.push("</" + col["fieldName"] + ">");
        }
        datasource.push("</"+this.dateItemName+">");
...
jetsennet.ui.GridList.prototype.createTable = function () {
    ...
    jQuery.each(this.tbodyTable.childNodes, function(index){
        ...
        jQuery(this).data("item", owner.objItems[index]);
    });
};
    ...
    if (menuControl == null) {
        ...
        headCheckMenu.addItem(jQuery.extend(new jetsennet.ui.MenuItem("全部选择"), { menuParam: checkName, onclick: function () {
            if (el(checkName + "-all") != null) {
                ...
                //-------------add by yyf-------------start-----------------------
                var trRow = jQuery("[name='" + checkName + "']");
                owner.selectedRows=[];
            	for(var vi=0;vi<trRow.length;vi++){
            		owner.selectedRows.push(vi);
            		jQuery(trRow[vi]).parent().parent().parent().addClass("selected");
            	}
                //-------------add by yyf--------------end----------------------
            };
        }}));
        headCheckMenu.addItem(jQuery.extend(new jetsennet.ui.MenuItem("反向选择"), { menuParam: checkName, onclick: function () {
            if (el(checkName + "-all") != null) {
                ...
                owner.selectedRows=[];
            	for(var vi=0;vi<trRow.length;vi++){
            		jQuery(trRow[vi]).parent().parent().parent().removeClass("selected");
            	}
                //---------add by yyf--------------------end--------------------
            };
        }}));
...
jetsennet.ui.GridList.prototype.formatDatasource = function() {
    ...
    var count = 0;
    jQuery.each(items, function(owner){     
    ...
    jQuery.each(this.columns, function(index) {
        ...
        if(this.fieldName) {
            ...
            if(this.format && jQuery.isFunction(this.format)) { //  格式化值
                ...
                cellValue = jetsennet.xml.htmlEscape(cellValue);
            }
...
jetsennet.ui.GridList.prototype.selectRow = function (iRowIndex, bMultiple) {
    ...
    var rows = this.tbodyTable.rows;
    var bSelect = true;
    if ((!this.enableMultiSelect)) {//(!bMultiple) || (!this.enableMultiSelect)
        this.colorEvenRows();
        this.selectedRows = [];
        //-----------add by yyf---------------start-------------
        $(this.bodyContainer).find("span.jetsen-checkbox").each(function(i) {
			    		if(i!=iRowIndex){
			    			if($(this).hasClass("jetsen-checkbox-chk-one")) {
			    				//console.debug($(this))
...
jetsennet.ui.GridList.prototype.assignEventHandlers = function () {
    ...
    jQuery(this.container).find("span.jetsen-checkbox").unbind("click").bind("click", function(e){
        ...
        if($this.hasClass("jetsen-checkbox-chk-all")) {
            ...
            if ($this.hasClass("checked")) {
           		 //-------------add by yyf------------------start---------------------------------
            	var trRow = jQuery("[name='" + $this.attr("checkname") + "']");
            	owner.selectedRows=[];
            	for(var vi=0;vi<trRow.length;vi++){
            		owner.selectedRows.push(vi);
            		jQuery(trRow[vi]).parent().parent().parent().addClass("selected");
            	...
            	owner.selectedRows=[];
            	for(var vi=0;vi<trRow.length;vi++){
            		owner.selectedRows.splice(vi, 1);
            		jQuery(trRow[vi]).parent().parent().parent().removeClass("selected");
            	}
            	//------------add by yyf-----------------------end-----------------------------
            ...
            if($this.hasClass("checked")){
            	...
            	var _rowindex = $this.parent().parent()[0].parentNode.rowIndex;
            	//owner.selectedRows.push(_rowindex);
            	owner.selectRow(_rowindex);
            }else{
            	//$this.parent().parent().parent().removeClass("selected");
            	var _rowindex = $this.parent().parent()[0].parentNode.rowIndex;
            	//owner.selectedRows.splice(_rowindex, 1);
            	owner.selectRow(_rowindex);
            }
            //------------add by yyf------------------end----------------------------
...
jetsennet.ui.GridList.prototype.addRow = function (rowItem, bBefore) {
    ...
    for (var i = 0; i < this.columns.length; i++) {
        ...
        row.appendChild(td);
        var cellValue = this.columns[i].format ? this.columns[i].format(rowItem[i], rowItem) : jetsennet.xml.htmlEscape(rowItem[i]);
