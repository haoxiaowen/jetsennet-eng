...
jetsennet.Crud = function(gridId, columns, pageBarId, orderBy,pageSize) {
    if (gridId) {
        var $this = this;
        this.grid = $.createGridlist(gridId, columns, pageBarId, orderBy, $.proxy(this.load, this),pageSize);//表格对象，自动生成
        this.grid.ondoubleclick = function(obj, el) {
            var id = $this.keyId.split(".");
            $this.edit(obj[id[id.length - 1]]);
        };
...
jetsennet.Crud.prototype.exportData = function(isAppendDate, options) {
    ...
    url += "?name=" + encodeURI(this.name);
    url += "&isAppendDate=" + !!isAppendDate;
    url += "&className=" + encodeURI(this.className);
    url += "&queryXml=" + encodeURI(this.getSqlQuery().toXml());
    ...
    if (!frame) {
        ...
        frame.onload = function() {
            if (!frame.contentWindow.document.body) {
                var result = frame.contentWindow.document.children[0].children;
                var errorCode = result[0].innerHTML;
                var errorString = result[1].innerHTML;
                if (errorCode == 50) {
