...
//    refeshMessage();
//    setInterval("refeshMessage()",1000*4*1);//1000为1秒钟，定时刷新消息图标
    MyApp.initToolBox();
	...
	obj.ID = "123456789";
	obj.NAME = "消息通知";
	obj.URL = "../../../jcom/logsystemweb/services_log.htm";
	MyApp.showIframe(obj,true);*/
//    MyApp.showIframe({ID:"main", NAME:"我的首页", URL:"main.htm"}, true);
    MyApp.showIframe({ID:"main", NAME:"我的首页", URL:"../../../jdlm/jdlmsystemweb/rentObj.htm"}, true);
    initPage();
...
function refeshMessage (){
	...
	var msgli =jQuery("#msgBox li ")[2];//取消息li
	if(gInt==0){//消息为0时，消息图标不点亮数字
		msgli.children[1].className="";
		msgli.lastChild.innerHTML="";
	}else{//消息不为0时，消息图片点亮
		if(msgli.children[1].className){
			msgli.lastChild.innerHTML=gInt;
		}else{
			msgli.children[1].className="badge"
			msgli.lastChild.innerHTML=gInt;
		}
