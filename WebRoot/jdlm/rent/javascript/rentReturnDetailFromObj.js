...
var gOutResCrud ;
var gResObj = jetsennet.queryString("objId") ;
function pageInit() { 
...
function resObjReturn(){
	var soncondition =[];
	var oneObj = gResObj;
	if(gResObj){
		var objarr =gResObj.split(",");
		if(objarr){
			oneObj=objarr[0];
		}
	}
	soncondition.push([ "obj.OBJ_ID", oneObj, jetsennet.SqlLogicType.And,jetsennet.SqlRelationType.Equal, jetsennet.SqlParamType.String ]);
	var sonjointables=[["ppn_rent_out_item", "item", "o.out_id=item.out_id ", jetsennet.TableJoinType.Inner],["PPN_RENT_OBJ", "obj", "obj.obj_code=item.item_code", jetsennet.TableJoinType.Inner]];
	var sonresultFields="max(o.out_create_time) OUT_CREATE_TIME";
	var sonqueryRes = SYSDAO.query("commonXmlQuery", "OUT_ID", "PPN_RENT_OUT",	"o", sonjointables, soncondition, sonresultFields);
	if (sonqueryRes && sonqueryRes.resultVal) {
		if (jetsennet.xml.deserialize(sonqueryRes.resultVal, "Record") != null) {
			var obj = null;
			obj = jetsennet.xml.deserialize(sonqueryRes.resultVal, "Record");
			if(obj.length>0){
				var conditions = [];
				conditions.push([ "ou.OUT_CREATE_TIME", obj[0].OUT_CREATE_TIME, jetsennet.SqlLogicType.And,jetsennet.SqlRelationType.Equal, jetsennet.SqlParamType.DateTime ]);
				conditions.push([ "o.OBJ_ID", gResObj, jetsennet.SqlLogicType.And,jetsennet.SqlRelationType.In, jetsennet.SqlParamType.String ]);
				var jointables =  [["ppn_rent_out_item", "item", "o.obj_code=item.item_code ", jetsennet.TableJoinType.Inner],["ppn_rent_out", "ou", "ou.out_id=item.out_id ", jetsennet.TableJoinType.Inner],
				                   ["ppn_cdm_column", "col", "col.COL_CODE=ou.out_coln_code", jetsennet.TableJoinType.Left],["uum_department", "dept", "trim(dept.depart_no) = trim(ou.out_dept_code)", jetsennet.TableJoinType.Left]];
				var resultFields = "ou.OUT_KEEPER,item.ITEM_OBJ_DESC,item.ITEM_OBJ_SPECS,item.item_obj_price,item.item_obj_currency,ITEM_OBJ_UOM,OBJ_CODE ITEM_OBJ_CODE,OBJ_NAME,OBJ_DESC ITEM_OBJ_DESC,OBJ_SPECS ITEM_OBJ_SPECS,OBJ_TYPE_ID ITEM_OBJ_TYPE,ou.out_dept_code,trim(dept.NAME) NAME,col.COL_NAME,ou.out_coln_code,ou.out_user_name,ou.out_user";
				/*var jointables =  [];
				var resultFields = "OBJ_CODE ITEM_OBJ_CODE,OBJ_NAME,OBJ_DESC ITEM_OBJ_DESC,OBJ_SPECS ITEM_OBJ_SPECS,OBJ_TYPE_ID ITEM_OBJ_TYPE";*/
				var queryRes = SYSDAO.query("commonXmlQuery", "OBJ_ID", "PPN_RENT_OBJ",	"o", jointables, conditions, resultFields);
				if (queryRes && queryRes.resultVal) {
					if (jetsennet.xml.deserialize(queryRes.resultVal, "Record") != null) {
						...
						if(len>0){
							...
							$("#RETU_MANAGE_USER").val(objout[0].OUT_USER_NAME);
							$("#RETU_DEPT_CODE").val(objout[0].NAME);
							$("#hid_RETU_COLUMN_CODE").val(objout[0].COL_CODE);
							$("#hid_RETU_MANAGE_USER").val(objout[0].OUT_USER);
							$("#hid_RETU_DEPT_CODE").val(objout[0].OUT_DEPT_CODE);
						}
						for (var i = 0; i < len; i++) {
							objout[i].ITEM_OBJ_NUM="1";
							generateTableFormRes(objout[i],true);
						}
					}
				}
			}
...
//	$("#RETU_DEPT_CODE").val("001");
	//复核人员
	$("#RETU_CHECK_USER").val(jetsennet.Application.userInfo.UserName);
...
function generateRetuFormOut(id){
	...
	var jointables =  [["ppn_rent_out_item", "item", "o.out_id=item.out_id ", jetsennet.TableJoinType.Inner],["ppn_rent_obj", "obj", "obj.obj_code=item.item_obj_code ", jetsennet.TableJoinType.Inner],
	                   ...
	                   ["uum_department", "dept", "trim(dept.depart_no) = o.out_dept_code", jetsennet.TableJoinType.Left]
	                   ];
		...
		$('#divdetail').find("table[class='tg']").each(function(i){
			...
			gResObj[i]=$("#resno"+tableNum).prop("value");
			 xml+="<TABLE CLASS_NAME='jetsennet.jdlm.beans.PpnRentInItem'>"
					...
					+"<ITEM_OBJ_NUM>"+ 1 +"</ITEM_OBJ_NUM>"
					+"<ITEM_OBJ_UOM>"+ $("#hid_ITEM_OBJ_UOM"+tableNum).prop("value") +"</ITEM_OBJ_UOM>"
					+"<ITEM_OBJ_UOM_SUM>"+ $("#hid_ITEM_OBJ_UOM"+tableNum).prop("value") +"</ITEM_OBJ_UOM_SUM>"
					...
					+"<ITEM_CHECK_REMARK_WG>"+ 1 +"</ITEM_CHECK_REMARK_WG>"
					+"<ITEM_CHECK_REMARK_JS>"+ 1 +"</ITEM_CHECK_REMARK_JS>"
					+"<ITEM_CHECK_BEFORE_WG>"+ 1 +"</ITEM_CHECK_BEFORE_WG>"
					+"<ITEM_CHECK_RESULT_WG>"+ 1 +"</ITEM_CHECK_RESULT_WG>"
					+"<ITEM_CHECK_BEFORE_JS>"+ 1 +"</ITEM_CHECK_BEFORE_JS>"
					+"<ITEM_CHECK_BEFORE_JS>"+ 1 +"</ITEM_CHECK_BEFORE_JS>"
					+"<ITEM_RESULT>"+ 1 +"</ITEM_RESULT>"
		    	...
		    	jetsennet.confirm("确定提交入库？", function() {
		    		SYSDAO.execute("commonObjsInsert", params, {
		    			success : function(resultVal) {
		    				updateRes();
		    				jetsennet.alert("成功!");
		    				window.parent.searchGoods();
		    		        window.parent.jetsennet.ui.Windows.close("retrun-object");
		    			},
		    			wsMode : "WEBSERVICE"
		    		}
