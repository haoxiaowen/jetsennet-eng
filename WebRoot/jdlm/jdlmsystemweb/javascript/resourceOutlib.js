...
var gSour = $.extend(new jetsennet.Crud("divApplicationList", gApplicationColumns, "divApplicationPage"), {
    ...
    oncateEditSet : function(obj) {
      el('txt_OUT_NAME').value = valueOf(obj, "OUT_NAME", "");
...
function saveadd(){
	 ...
	 //出库单名称
	 if(el('txt_OUT_NAME').value==null||el('txt_OUT_NAME').value==""){
		 jetsennet.alert("出库但名称,不能为空");
		 return true;
	 }
	 //库管人员
...
function onaddSource(){
	return {
		...
		OUT_NAME:el('txt_OUT_NAME').value,//出库单名称
...
function chooseSour(){
	var result = SYSDAO.queryObjs('commonXmlQuery', 'p.ord_id', 'PPN_RENT_ORD', "p",null, null, 'p.*');
	  if (result) {
	    	$('#txt_chooseSource').empty();
	        var len = result.length;
	        jQuery("<option selected='selected' >全部</option>", {}).attr("value", 0).appendTo($("#txt_chooseSource"));
	        for (var i = 0; i < len; i++) {
	        	jQuery("<option>"+result[i].ORD_CODE+"</option>", {}).attr("value", result[i].ORD_ID).appendTo($("#txt_chooseSource"));
	        }
	    }
}
function getChooseSour(obj){
	var ordid = obj.value;
	var ret = SYSDAO.query("commonXmlQuery", "p.obj_id", "PPN_RENT_ORD_ITEM", "t", 
			[["PPN_RENT_ORD", "p", "t.ord_id = p.ord_id", jetsennet.TableJoinType.Left]],
	  		[["p.ord_id",ordid, jetsennet.SqlLogicType.And,jetsennet.SqlRelationType.Equal,jetsennet.SqlParamType.String]],	
	  		"t.*,p.*");
	if(ret){
		var _obj = jetsennet.xml.deserialize(ret.resultVal, "Record");
		if(_obj){
			el('txt_OUT_CODE').value = _obj[0].ORD_CODE;
			el('txt_OUT_DEPT_CODE').value = _obj[0].ORD_DEPT_CODE;
			el('txt_OUT_COLN_CODE').value = _obj[0].ORD_COLUMN_CODE;
			el('txt_OUT_NAME').value = _obj[0].ORD_NAME;
			el('txt_OUT_DESC').value = _obj[0].ORD_DESC;
			
			el('txt_OBJ_TYPE_NAME').value = _obj[0].ITEM_OBJ_TYPE;
			el('txt_ITEM_CODE').value = _obj[0].ITEM_CODE;
			el('txt_ITEM_NAME').value = _obj[0].ITEM_NAME;
			el('txt_ITEM_OBJ_PRICE').value = _obj[0].ITEM_OBJ_PRICE;
			el('txt_ITEM_OBJ_NUM').value = _obj[0].ITEM_OBJ_NUM;
			el('txt_ITEM_REMARK').value = _obj[0].ITEM_REMARK;
			el('txt_ITEM_OBJ_SPECS').value = _obj[0].ITEM_OBJ_SPECS;
			el('txt_ITEM_OBJ_UOM').value = _obj[0].ITEM_OBJ_UOM;
			el('txt_ITEM_OBJ_DESC').value = _obj[0].ITEM_OBJ_DESC;
			el('txt_ITEM_OBJ_CODE').value = _obj[0].ITEM_OBJ_CODE;
		}
	}
}